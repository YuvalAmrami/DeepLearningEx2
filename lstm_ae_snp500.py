...
parser.add_argument('--stats_file_prefix',  default='stas_snp', help='output file for statistics')
parser.add_argument('--model_file_prefix',  default='mod_snp', help='output file for model')
...
logging.basicConfig(filename="lstm_ae_train.log", filemode='w', format='%(asctime)s %(levelname)-8s %(message)s',
...
if is_prediction_task:
    ...
    model.to(device)
    train, val, test = DU.split_stocks_dataset(stocks, 1)

else:
    model = LSTM_AE_Model(device, input_size=input_size, hidden_dim=hidden_dim, seq_len=seq_len)
    model.to(device)
    classification_criterion = nn.CrossEntropyLoss()
    train, val, test = DU.split_stocks_dataset(stocks, attempt_name)
