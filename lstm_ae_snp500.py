...
from lstm_model_predicter import LSTM_AE_Model_pred
...
parser.add_argument('--lr', type=float, default=10**-5, help='learning rate')
...
parser.add_argument('--stats_file_prefix',  default='stas_snp_01E', help='output file for statistics')
parser.add_argument('--model_file_prefix',  default='mod_snp_01E', help='output file for model')
parser.add_argument('--input_size', default=1, type=int, help='input size')
parser.add_argument('--attempt_name', default='snp_01E', help='name for train, validation and test files of this attempt')
...
for train_name_index, val_name_index in kf2.split(data_rest):
    train_name, val_name = data[train_name_index], data[val_name_index]
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        if is_prediction_task:
            ...
            if val_acc > curr_best_accuracy:
                curr_best_accuracy = val_acc
    ...
    if is_prediction_task:
        curr_test_loss = model.evaluate_model(dataset_test, device, criterion, classification_criterion, input_size, seq_len)
    else:
